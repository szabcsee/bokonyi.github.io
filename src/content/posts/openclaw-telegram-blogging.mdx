...
Bender tried four different image generation APIs. All of them failed — rate limits, quota issues, auth problems. Welcome to the life of AI tooling.

So we used an existing image as a placeholder. I'll swap it when the APIs recover. But the *intention* was clear: describe the image you want, get a style match, generate it inline with the post creation. That's the workflow I want. We're just not quite there yet on reliability.
...
**The memory thing is real.** Bender doesn't just have a conversation — it reads my memory files before responding. So it knows that I run digitalyogis Kft, that I prefer value-based pricing over hourly, that I have a background in frontend but now mostly do AI automation work. It doesn't waste time explaining things I already know, and it adapts to my communication style.
...
**Voice output is underrated.** Bender can generate voice (via ElevenLabs) and send audio messages. I've started using this for longer outputs — it reads the post back to me while I'm walking. Way more pleasant than squinting at Telegram messages.

---

## What's still broken

**Image generation is unreliable.** Four APIs, four failures. I understand this is a tooling issue — the APIs have limits and I've burned through my free tier quotas. But I want this to be a one-step thing: write post + generate cover image in one go. Currently it's write post + cross fingers + maybe image generates.
...
The blog post you're reading right now? Written via Telegram. Generated image attempt (Hellblazer-style noir robot at a laptop — just imagine it) failed due to API limits, so I used a placeholder. That's the current state of the art: pretty good text, flaky image gen, still better than doing it myself.
